...
fn addLogger(b: *std.Build, target: std.Build.ResolvedTarget, mode: std.builtin.OptimizeMode) !void {
    ...
    if (b.option(bool, "ssl", "enable ssl") orelse false) {
        ...
        // const mod_iguana = b.dependency("iguana", .{
        //     .target = target,
        //     .optimize = optimize,
        // }).module("SimpleLib"));

        // exe.addPackage("iguana", b.path(iguana_index_file));
        // exe.addPackage("ssl", "iguanassl.zig");

        // exe.root_module.addAnonymousImport("ssl", .{
        //     .root_source_file = b.path("iguanassl.zig"),
        //     .dependencies = &[_]std.build.Pkg {
        //         .{ .name = "iguana", .source = b.path(iguana_index_file) },
        //     },
        // });

        // const table_helper = b.addModule("ssl", .{
        //     .root_source_file = b.path(iguana_index_file)
        // });
        // exe.addModule("ssl", table_helper);

        exe.root_module.addImport("ssl", b.createModule(.{
            .root_source_file = b.path(iguana_index_file),
        }));

    } else {
        exe.root_module.addAnonymousImport("ssl", .{
            .root_source_file = b.path("nossl.zig"),
        });
    }
...
// fn addPublisher(b: *std.Build, target: std.Build.ResolvedTarget, mode: std.builtin.OptimizeMode) !void {
